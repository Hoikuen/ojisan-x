...
> 🎞 **アニメーション（歩き等）を付けたい動きは「連番」で複数コマ発注する**：
> 例）主人公の歩行 → `ojisan_walk_1` `ojisan_walk_2` `ojisan_walk_3` `ojisan_walk_4`（2〜4枚）。
> 各コマは「右足前／中間／左足前／中間」のように脚の位置を変える。同一キャラ・同一画風・同一サイズ感で。
> `extract_raw.py` が連番を共通キャンバス（足元揃え）に整列するので、置いて実行するだけで歩行アニメになる。
> （詳細は `RESKIN.md`）
