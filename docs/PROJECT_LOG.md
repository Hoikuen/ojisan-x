...
### 5. 実機フィードバック対応（同日・その3）
- **背景の地面ズレを修正**（GameScene.js）：背景画像は1536x864だが画面は600px高で、従来は上600pxだけ表示＝**画像下部の地面帯が切れて主人公/敵が浮いて(埋まって)見えていた**。`bg.tilePositionY = texH - GAME_H` で画像下端=地面を画面下端に合わせ、全5フロアで接地表示に。スクショで1〜5全確認・18テスト緑。
  - ユーザーが言った「ステージ2の敵が埋まってない」はこれが原因（敵は実アート＆足元575で正しく接地。差し替え不要）。
- **キック差し替えの発注強化**（ORDER_PROMPTS_CODEX player_attack_sheet）：旧キックが読みづらいため「水平前蹴り・脚と胴の間に明確な隙間・一目で蹴りと分かるシルエット」を明記。**次のCodex再生成→再抽出→再デプロイで反映**（現状の絵はまだ旧キック）。
- **未了の検討**：より本格的な横スク背景にするなら「①継ぎ目なくタイルする遠景バックドロップ＋②独立した地面ストリップ(FLOOR_Yに敷く)」の2層分離を再発注する案あり（今回は既存フルシーン絵の地面帯を出す簡易修正で対応）。

### 6. 攻撃アニメが「当てコマで止まらない」問題を修正（同日・その4）
- 症状：キック/パンチを押すと"違う動作"に見える。**配線は正常**（トレースでキック→anim_pn_kick確認）だが、3コマ攻撃(構え→当て→戻し)が`repeat:0`で**最後の戻しコマで停止保持**するため、良い当てコマ(kick=コマ5/punch=コマ2)が一瞬で流れて残らなかった。
...
### 8. 実機FB：ハゲ化引き継ぎ／ボス撃破→退職金ドロップ／変身テキスト（同日・その6）
- **ハゲ化のフロア跨ぎ引き継ぎ**：クリア時にハゲ化中なら残り時間を次フロアへ。`_advanceFloor`が`poweredMs`をscene.startで渡し、`init`で受け→create後に`player.powerUp(ms)`。`Player.powerUp(ms)`に残り時間引数を追加。実機トレースで floor2 powered=true(残7151ms)確認。
- **ボス撃破フローを刷新**：旧「白い残骸が残り自動進行」→新「ボス本体をα0でフェードアウト＋`退職金(itemCash×1.7)`をドロップ→拾うと『FLOOR CLEAR!』→次フロア」。`onBossDefeated`を書き換え＋`_spawnRetirementDrop`/`_collectRetirement`追加。15秒で自動進行のソフトロック防止つき。近接撃破時はドロップ隣接で即回収、遠距離撃破時は歩いて回収（プロンプト『退職金を回収せよ！』）。
- **変身テキスト**：パワーアップ時 `ブチギレ！！`→`ハゲまくり！！`。
- 18テスト緑・トレースでドロップ生成/回収/進行/引き継ぎ/エラー0を確認。

### 9. bald(ハゲ親父)を組込（同日・その7）
- Codex納品が**14コマ仕様でなく「fury 6ポーズ1枚」**で来た（しかも別repo ojisan-quest に保存される事故）。画風は良好（赤禍・湯気・炎オーラ・新主人公と一致）。
- 6ポーズ(f1湯気待機/f2前傾/f3踏込/f4炎オーラ/f5構え/f6パンチ)を**baldの14スロットにマッピング**して組込：idle=f1↔f4(激怒脈動)、walk=f2/f3踏込マーチ、punch=f5→f6(完璧)、kick=f5→f6流用、jump=f4、被弾は変身中=無敵で非表示。図形高さ295pxで通常版とスケール一致。
- 発注書(ORDER_PROMPTS_CODEX)に**出力先フルパス＋repo取り違え防止＋14コマ厳守**を追記済み。元シートは `raw_generated/bald_fury_sheet.png`（手元のみ）。
- 18テスト緑・実機で変身表示確認。

### 残タスク
- 流用3体(fruit/banana/buddha)の本発注（任意）。音は本物素材への差し替え推奨。背景2層化（任意）。bald歩き/ジャンプは流用なので、本物の歩き等が欲しければ14コマで再発注。
